        ...
        {filtered.length === 0 ? (
          ...
          filtered.map(r => (
            <TouchableOpacity
              key={r._id}
              style={s.item}
              onPress={() => r.status === 'pre_aprobado'
                ? router.push(`/(auditor)/aprobacion/${r._id}` as any)
                : undefined
              }
            >
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.itemSub}>Enc: {r.encargadoNombre} · {r.horasTotalesDec}h · {r.kmRecorridos} km</Text>
              </View>
